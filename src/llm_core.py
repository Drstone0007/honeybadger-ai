...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Honey Badger' canonical OpenAI-style messages to native Ollama /api/chat.

    Honey Badger carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/pewdiepie-archdaemon/honeybadger")
        h.setdefault("X-OpenRouter-Title", "Honey Badger")
    return h
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Honey Badger-only metadata before sending messages to providers.
