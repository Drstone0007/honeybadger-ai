...
# Override at deploy time via HONEYBADGER_MAIL_ATTACHMENTS_DIR. Defaults to a
# subdir of the install's data/ tree so the app works out-of-the-box without
# a hardcoded /home/<user>/ path.
ATTACHMENTS_DIR = Path(os.environ.get("HONEYBADGER_MAIL_ATTACHMENTS_DIR", str(DATA_DIR / "mail-attachments")))
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "honeybadger_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN honeybadger_kind TEXT")
    except Exception:
        pass
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("HONEYBADGER_IMAP_TIMEOUT_SECONDS"))
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for Honey Badger-generated mail (e.g. reminder, scheduled).
    honeybadger_kind: Optional[str] = None
