                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Honey Badger (if public URL is configured).
                                    ...
                                    alert_body = (
                                        ...
                                        f"Reason: {reason}\n\n"
                                        + (f"Open in Honey Badger: {open_url}\n\n" if open_url else "")
                                        + f"---\n"
                                        f"From: {sender}\n"
                                        f"Subject: {subject}\n"
                                        f"Date: {msg.get('Date','')}\n\n"
                                    ...
                                    # HTML alternative with a clickable "Open in Honey Badger" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "honeybadger_kind" if "honeybadger_kind" in cols else "'scheduled' AS honeybadger_kind"
        rows = conn.execute(f"""
        ...
        for r in rows:
            ...
            try:
                ...
                honeybadger_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Honey Badger-Origin"] = "honeybadger-ui"
                outer["X-Honey Badger-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", honeybadger_kind or "scheduled")[:64]
                outer["X-Honey Badger-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `honeybadger-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `HONEYBADGER_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `honeybadger-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("HONEYBADGER_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `HONEYBADGER_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (HONEYBADGER_INPROCESS_POLLERS=0); "
            "drive `honeybadger-mail poll-scheduled` externally."
        )
        return
    import asyncio
