...
# Override at deploy time via `HONEYBADGER_FALLBACK_OWNER` env var. In a real
# multi-user install set `HONEYBADGER_SINGLE_USER=0` so unauthenticated requests
# are rejected instead of silently writing to this address.
import os as _os
FALLBACK_OWNER = _os.environ.get("HONEYBADGER_FALLBACK_OWNER", "owner@localhost")
_SINGLE_USER_MODE = _os.environ.get("HONEYBADGER_SINGLE_USER", "1") != "0"
    ...
    async def export_ics(request: Request, cal_id: str):
        ...
        try:
            ...
            lines = [
                "BEGIN:VCALENDAR",
                "VERSION:2.0",
                "PRODID:-//Honey Badger//Calendar//EN",
