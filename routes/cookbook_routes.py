...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[honeybadger] HF token: applied"; '
    'else '
    'echo "[honeybadger] HF token: NOT SET — gated/private models will be denied. '
    'Add one in Honey Badger Settings -> Cookbook -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            proc = await asyncio.create_subprocess_exec(
                ssh_keygen, "-t", "ed25519", "-N", "", "-C", "honeybadger-cookbook", "-f", str(key_path),
    ...
    def _user_shell_path_bootstrap() -> list[str]:
        return [
            'HONEYBADGER_USER_SHELL="${SHELL:-}"',
            'if [ -n "$HONEYBADGER_USER_SHELL" ] && [ -x "$HONEYBADGER_USER_SHELL" ]; then',
            '  HONEYBADGER_USER_PATH="$("$HONEYBADGER_USER_SHELL" -ic \'printf "__HONEYBADGER_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__HONEYBADGER_PATH__//p\' | tail -n 1 || true)"',
            '  if [ -n "$HONEYBADGER_USER_PATH" ]; then export PATH="$HONEYBADGER_USER_PATH:$PATH"; fi',
            'fi',
        ]
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When Honey Badger runs from a venv (e.g. native macOS install), put its bin
        ...
        if remote and is_windows:
            ...
            ps_lines = []
            ps_lines.append('$sessionDir = "$env:TEMP\\honeybadger-sessions"')
            ...
            launch_ps = (
                "$sd = \\\"$env:TEMP\\honeybadger-sessions\\\"; "
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        if host:
            ...
            # LOCAL scan: use sys.executable (the venv Python Honey Badger is already
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            runner_lines = ["#!/bin/bash"]
            runner_lines.extend(_user_shell_path_bootstrap())
            runner_lines.append('HONEYBADGER_PREFLIGHT_EXIT=""')
            # Put Honey Badger's own venv bin on PATH (local runs only) so the serve
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('    HONEYBADGER_PREFLIGHT_EXIT=127')
                runner_lines.append('  fi')
                runner_lines.append('fi')
                ...
                runner_lines.append(f'HONEYBADGER_OLLAMA_HOST={_bash_squote(_ollama_host)}')
                runner_lines.append(f'HONEYBADGER_OLLAMA_PORT="{_ollama_port}"')
                runner_lines.append('HONEYBADGER_OLLAMA_URL=""')
                runner_lines.append('for _ody_ollama_try in $(seq 1 20); do')
                runner_lines.append('  for _ody_ollama_port in "$HONEYBADGER_OLLAMA_PORT" 11434; do')
                ...
                runner_lines.append('        HONEYBADGER_OLLAMA_URL="$_ody_ollama_url"')
                runner_lines.append('        HONEYBADGER_OLLAMA_PORT="$_ody_ollama_port"')
                runner_lines.append('        break 3')
                runner_lines.append('      fi')
                runner_lines.append('    done')
                runner_lines.append('  done')
                runner_lines.append('  [ "$_ody_ollama_try" -eq 1 ] && echo "[honeybadger] Waiting for an existing Ollama API on ports ${HONEYBADGER_OLLAMA_PORT}/11434..."')
                runner_lines.append('  sleep 1')
                runner_lines.append('done')
                runner_lines.append('if [ -n "$HONEYBADGER_OLLAMA_URL" ]; then')
                runner_lines.append('  if [ "$HONEYBADGER_OLLAMA_PORT" != "' + _ollama_port + '" ]; then')
                runner_lines.append('    echo "[honeybadger] Selected Ollama port ' + _ollama_port + ' was not reachable; using running Ollama on port ${HONEYBADGER_OLLAMA_PORT}."')
                runner_lines.append('  fi')
                runner_lines.append('  echo "[honeybadger] Ollama API ready on port ${HONEYBADGER_OLLAMA_PORT}: ${HONEYBADGER_OLLAMA_URL}"')
                runner_lines.append('  echo "[honeybadger] This task is monitoring an existing Ollama server; stopping it here will not stop an external Docker/system service."')
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('if ! command -v ollama &>/dev/null; then')
                runner_lines.append('  echo "ERROR: Ollama not found and no Ollama API is reachable on 127.0.0.1, localhost, or host.docker.internal (ports ${HONEYBADGER_OLLAMA_PORT}/11434)."')
                ...
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('HONEYBADGER_OLLAMA_URL="http://${HONEYBADGER_OLLAMA_HOST}:${HONEYBADGER_OLLAMA_PORT}"')
                if remote and _ollama_host in ("0.0.0.0", "::"):
                    runner_lines.append('echo "[honeybadger] WARNING: remote Ollama will bind to ${HONEYBADGER_OLLAMA_HOST}:${HONEYBADGER_OLLAMA_PORT} so Honey Badger can reach it from this host."')
                    runner_lines.append('echo "[honeybadger] Ollama has no built-in authentication; expose this only on a trusted LAN/VPN or provide an explicit OLLAMA_HOST with your own access controls."')
                runner_lines.append('echo "Starting ollama server on ${HONEYBADGER_OLLAMA_HOST}:${HONEYBADGER_OLLAMA_PORT}..."')
                runner_lines.append('OLLAMA_HOST="${HONEYBADGER_OLLAMA_HOST}:${HONEYBADGER_OLLAMA_PORT}" ollama serve')
                ...
                runner_lines.append('  HONEYBADGER_PREFLIGHT_EXIT=1')
                runner_lines.append('fi')
                ...
                runner_lines.append('  HONEYBADGER_PREFLIGHT_EXIT=127')
                runner_lines.append('elif ! HONEYBADGER_SGLANG_IMPORT_ERROR="$(python3 -c "import sglang" 2>&1)"; then')
                runner_lines.append('  echo "ERROR: SGLang is installed but failed to import."')
                runner_lines.append('  printf "%s\\n" "$HONEYBADGER_SGLANG_IMPORT_ERROR"')
                runner_lines.append('  HONEYBADGER_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
            elif "scripts/diffusion_server.py" in req.cmd or ".diffusion_server.py" in req.cmd:
                runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
                runner_lines.append('if ! HONEYBADGER_DIFFUSION_IMPORT_ERROR="$(python3 -c "import torch, diffusers" 2>&1)"; then')
                runner_lines.append('  echo "ERROR: Diffusion serving requires PyTorch + diffusers."')
                runner_lines.append('  printf "%s\\n" "$HONEYBADGER_DIFFUSION_IMPORT_ERROR"')
                runner_lines.append('  HONEYBADGER_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if platform == "windows":
            ...
            setup_script = (
                'powershell -Command "'
                "New-Item -ItemType Directory -Force -Path $env:TEMP\\honeybadger-sessions | Out-Null; "
