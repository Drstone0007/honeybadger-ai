...
const GROUP_STATE_KEY = 'honeybadger-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const gids = JSON.parse(localStorage.getItem('honeybadger-group-sessions') || '[]');
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('honeybadger-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          // Text delta (Honey Badger format)
