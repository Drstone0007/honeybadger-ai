...
const _ACTION_RECENTS_KEY = 'honeybadger-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'honeybadger-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Honey Badger';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Honey Badger' : shortModel(resolvedModel);
