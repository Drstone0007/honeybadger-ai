...
let _sortMode = Storage.get('honeybadger-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Honey Badger Chat';
...
const FOLDER_STATE_KEY = 'honeybadger-folder-state';
const FOLDER_ORDER_KEY = 'honeybadger-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Honey Badger kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Honey Badger Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('honeybadger-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('honeybadger-session-sort', mode);
  else Storage.remove('honeybadger-session-sort');
  renderSessionList();
}
