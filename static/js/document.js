...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__honeybadgerActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'honeybadger-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'honeybadger-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__honeybadgerActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('honeybadger-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('honeybadger-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'honeybadger-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('honeybadger-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('honeybadger-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('honeybadger-suggestions-' + docId);
      if (!raw) return;
