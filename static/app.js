// ============================================
// Honey Badger UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__honeybadgerDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Honey Badger Chat';
  ...
  if (_opener) window._honeybadgerRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('honeybadger-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'honeybadger-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Honey Badger...');
      }
  ...
  const UI_VIS_KEY = 'honeybadger-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'honeybadger-toolbar-visibility';
    try {
...
// ============================================
function startHoney BadgerApp() {
  if (window.__honeybadgerAppStarted) return;
  window.__honeybadgerAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('honeybadger-doc-open-' + _curSession) === '1') {
  ...
  window._honeybadgerBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._honeybadgerRouteOpener) {
          try { window._honeybadgerRouteOpener(); } catch (_) {}
          window._honeybadgerRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startHoney BadgerApp, { once: true });
} else {
  startHoney BadgerApp();
}
